...
def fit_and_predict(df_train, df_test, params={}, verbose=True, n_best_features=50):
    importances = sorted(
        joblib.load("cache/lgb_bin_importance.pkl").items(),
    ...
    name = "lgb_bin_trun_{}_{:.6f}".format(
    ...
    df_val_preds.to_pickle("cache/lgb_bin_trun_val.pd")
    joblib.dump(np.mean(test_preds, axis=0), "cache/lgb_bin_trun_test.pkl")
    joblib.dump(global_importance, "cache/lgb_bin_trun_importance.pkl")
